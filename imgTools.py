...
##############################################################################
    ...
    for i in range(npulses): ## 펄스의 개수만큼 반복문
        ...
        dr_i = norm(r0) - norm(r - r0, axis=0) ## r0를 norm을 취하면 원점에서 펄스까지의 거리가 나옴. 이 값에서 펄스-픽셀 거리를 계산한 값을 빼서 거리 값을 구한다?
...
##############################################################################
    ...
    u = np.arange(-nu/2, nu/2)*du # sample간 의 range resoultion 간격
    v = np.arange(-nv/2, nv/2)*dv # pulse 간의 range resoultuon 간격
    #u_start = 0
    #u_end = 1000
    #v_start = 0
    #v_end=1000
